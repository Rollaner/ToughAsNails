...
public class TileEntityTemperatureSpread extends TileEntity implements ITickable, ITemperatureRegulator {
	...
	// TODO: Stagger updates if necessary, so verification occurs slower away
	...
	public void update() {
		...
		if (++updateTicks % 20 == 0) {
			...
			// When first placed, this may be null because it hasn't been
			// created when read from NBT
			if (this.maxSpreadBox == null) {
				this.maxSpreadBox = new AxisAlignedBB(this.pos.getX() - MAX_SPREAD_DISTANCE,
						this.pos.getY() - MAX_SPREAD_DISTANCE, this.pos.getZ() - MAX_SPREAD_DISTANCE,
						this.pos.getX() + MAX_SPREAD_DISTANCE, this.pos.getY() + MAX_SPREAD_DISTANCE,
						this.pos.getZ() + MAX_SPREAD_DISTANCE);
			}
			...
			for (EntityPlayer player : world.getEntitiesWithinAABB(EntityPlayer.class, this.maxSpreadBox)) {
				...
				// Check if the player collides with any of the filled
				// positions.
				// Player must be in a strength equal to or less than the
				// distance they are away from the coil
				outer: for (int i = MAX_SPREAD_DISTANCE - distance; i >= 0; i--) {
					for (BlockPos pos : this.filledPositions[i]) {
						// If a collision is found, stop looking
						if (player.getEntityBoundingBox().intersects(pos.getX(), pos.getY(), pos.getZ(),
			...
			if (ENABLE_DEBUG) {
				// There is a mismatch between the filled positions and the
				// spawned entities, repopulate spawned entities
				// If this is active, there should at least be a position in the
				// set for the max spread distance
				if (!world.isRemote && spawnedEntities.isEmpty()
						&& !this.filledPositions[MAX_SPREAD_DISTANCE].isEmpty()) {
					for (int strength = 0; strength <= MAX_SPREAD_DISTANCE; strength++) {
						for (BlockPos pos : this.filledPositions[strength]) {
							final AxisAlignedBB boundingBox = new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(),
							...
							Predicate<EntitySmallFireball> predicate = new Predicate<EntitySmallFireball>() {
								...
								public boolean apply(EntitySmallFireball input) {
									// Check intersections with this entity a
									// little bit over 1x1x1, because it seemed
									// the outer layer of fireballs wasn't
									// included otherwise
									BlockPos pos = input.getPosition();
									return boundingBox.intersects(pos.getX() - 0.1D, pos.getY() - 0.1D,
											pos.getZ() - 0.1D, pos.getX() + 1.1D, pos.getY() + 1.1D, pos.getZ() + 1.1D);
								}
							};

							// Fireballs don't have a bounding box so we can't
							// use getEntitiesWithinAABB (which really stinks!)
	...
	public void fill() {
		...
		for (EnumFacing facing : EnumFacing.values()) {
			...
			// Only attempt to update tracking for this position if there is air
			// here.
			...
			for (Set<BlockPos> trackedPositions : this.filledPositions) {
				for (BlockPos trackedPosition : trackedPositions) {
					if (trackedPosition != null) {
						...
						EntitySmallFireball fireball = new EntitySmallFireball(getWorld(), (double) pos.getX() + 0.5D,
								(double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D, 0.0D, 0.0D, 0.0D);
	...
	/**
	 * Begins tracking this position or updates its strength. Returns true if
	 * changed from before
	 **/
	...
	/**
	 * Strength is the strength of the initial pos, not what it will spread to
	 * its surroundings
	 */
	...
	private boolean canFill(BlockPos pos) {
		// Only spread within enclosed areas, significantly reduces the impact
		// on performance and suits the purpose of coils
		return !this.getWorld().isBlockFullCube(pos) && (!this.getWorld().canSeeSky(pos)
				|| this.getWorld().getBlockState(pos).getBlock() instanceof IDecayableCrop);
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		if (compound.hasKey("FilledPositions")) {
			...
			for (int strength = 0; strength <= MAX_SPREAD_DISTANCE; strength++) {
				if (!filledCompound.hasKey("Strength" + strength))
					throw new IllegalArgumentException(
							"Compound missing strength sub-compound Strength" + strength + "!");
			...
			this.spawnedEntities = Sets.newConcurrentHashSet(); // Recreate
																// spawned
																// entities set
																// and
																// repopulate
	...
	public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newSate) {
		// This should function as it does in Vanilla, using Forges setup
		// appears to break fireball spawning when the state is changed
	...
	public boolean isPosRegulated(BlockPos pos) {
		for (int i = 0; i < MAX_SPREAD_DISTANCE; i++) {
			...
			if (regulatedPositions.contains(pos))
				return true;
		}
