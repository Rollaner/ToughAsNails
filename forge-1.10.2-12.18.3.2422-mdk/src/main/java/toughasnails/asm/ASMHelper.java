...
public class ASMHelper {
	...
	public static MethodInsnNode getUniqueMethodInsnNode(MethodNode methodNode, int opcode, String owner,
		...
		if (matchedMethodNodes.isEmpty())
			throw new RuntimeException(
					"No method instruction node found matching " + owner + " " + names[0] + " " + desc);
		if (matchedMethodNodes.size() > 1)
			LOGGER.warn("Too many matched instructions were found in " + methodNode.name + " for " + owner + " "
					+ names[0] + " " + desc + ". Crashes or bugs may occur!");

		return matchedMethodNodes.get(matchedMethodNodes.size() - 1);
	}

	public static List<MethodInsnNode> matchMethodInsnNodes(MethodNode methodNode, int opcode, String owner,
			String[] names, String desc) {
		...
		for (AbstractInsnNode insnNode : methodNode.instructions.toArray()) {
			if (insnNode instanceof MethodInsnNode && insnNode.getOpcode() == opcode) {
				...
				if (methodInsnNode.owner.equals(owner) & validMethodNames.contains(methodInsnNode.name)
	...
	public static void verifyClassHash(String className, byte[] bytes, String... expectedHashes) {
		...
		if (!Lists.newArrayList(expectedHashes).contains(currentHash)) {
			String error = String.format("Unexpected hash %s detected for class %s. Crashes or bugs may occur!",
