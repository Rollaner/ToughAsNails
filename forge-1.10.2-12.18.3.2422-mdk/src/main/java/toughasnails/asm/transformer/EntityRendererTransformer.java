...
public class EntityRendererTransformer implements IClassTransformer {
	private static final String[] VALID_HASHES = new String[] { "7039efd63c08f2d8fa9f000a4a194d5c",
			"48321722b6b3220fc8d2b5dd4a703476" };
	...
	private static final String[] GET_FLOAT_TEMPERATURE_NAMES = new String[] { "getFloatTemperature", "func_180626_a",
			"a" };
	...
	private byte[] transformEntityRenderer(byte[] bytes, boolean obfuscatedClass) {
		...
		for (MethodNode methodNode : classNode.methods) {
			if (ASMHelper.methodEquals(methodNode, RENDER_RAIN_SNOW_NAMES, "(F)V")) {
				MethodInsnNode targetMethodInsnNode = ASMHelper.getUniqueMethodInsnNode(methodNode,
						Opcodes.INVOKEVIRTUAL, ObfHelper.unmapType(obfuscatedClass, "net/minecraft/world/biome/Biome"),
						GET_FLOAT_TEMPERATURE_NAMES,
						ObfHelper.createMethodDescriptor(obfuscatedClass, "F", "net/minecraft/util/math/BlockPos"));
				...
				targetMethodInsnNode.desc = ObfHelper.createMethodDescriptor(obfuscatedClass, "F",
						"net/minecraft/world/biome/Biome", "net/minecraft/util/math/BlockPos");
		...
		if (successfulTransformations.size() != 2)
			throw new RuntimeException("An error occurred transforming EntityRenderer. Applied transformations: "
