...
public class WorldTransformer implements IClassTransformer {
	...
	private static final String[] CAN_SNOW_AT_NAMES = new String[] {
			"canSnowAt", "func_175708_f", "f" };
	private static final String[] CAN_BLOCK_FREEZE_NAMES = new String[] {
			"canBlockFreeze", "func_175670_e", "e" };
	private static final String[] IS_RAINING_AT_NAMES = new String[] {
			"isRainingAt", "func_175727_C", "B" };
	private static final String[] GET_BIOME_GEN_FOR_COORDS_NAMES = new String[] {
			"getBiome", "func_180494_b", "b" };
	...
	private byte[] transformWorld(byte[] bytes, boolean obfuscatedClass) {
		...
		for (MethodNode methodNode : classNode.methods) {
			if (ASMHelper.methodEquals(methodNode, CAN_SNOW_AT_NAMES,
					ObfHelper.createMethodDescriptor(obfuscatedClass, "Z",
							"net/minecraft/util/math/BlockPos", "Z"))) {
				InsnList insnList = new InsnList();

				// Get the current season
				insnList.add(new VarInsnNode(Opcodes.ALOAD, 0));
				insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
						"toughasnails/api/season/SeasonHelper", "getSeasonData",
						ObfHelper.createMethodDescriptor(obfuscatedClass,
								"toughasnails/api/season/ISeasonData",
								"net/minecraft/world/World"),
						false));
				insnList.add(new MethodInsnNode(Opcodes.INVOKEINTERFACE,
						"toughasnails/api/season/ISeasonData", "getSubSeason",
						"()Ltoughasnails/api/season/Season$SubSeason;", true));
				insnList.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
						"toughasnails/api/season/Season$SubSeason", "getSeason",
				...
				insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
						"toughasnails/season/SeasonASMHelper",
						"canSnowAtInSeason",
						ObfHelper.createMethodDescriptor(obfuscatedClass, "Z",
								"net/minecraft/world/World",
								"net/minecraft/util/math/BlockPos", "Z",
								"toughasnails/api/season/Season"),
						false));
				...
				if (ASMHelper.methodEquals(methodNode, CAN_BLOCK_FREEZE_NAMES,
						ObfHelper.createMethodDescriptor(obfuscatedClass, "Z",
								"net/minecraft/util/math/BlockPos", "Z"))) {
				InsnList insnList = new InsnList();
				...
				successfulTransformations
						.add(methodNode.name + " " + methodNode.desc);
			} else if (ASMHelper.methodEquals(methodNode, IS_RAINING_AT_NAMES,
					ObfHelper.createMethodDescriptor(obfuscatedClass, "Z",
				...
				insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
						...
						ObfHelper.createMethodDescriptor(obfuscatedClass, "Z",
								...
								"toughasnails/api/season/Season"),
						false));
				insnList.add(new InsnNode(Opcodes.ICONST_0)); // Necessary for
																// compatibility
																// with
																// RandomThingsCore
				insnList.add(new InsnNode(Opcodes.IRETURN));

				MethodInsnNode invokeMethodNode = ASMHelper
						.getUniqueMethodInsnNode(methodNode,
								Opcodes.INVOKEVIRTUAL,
								ObfHelper.unmapType(obfuscatedClass,
										"net/minecraft/world/World"),
								GET_BIOME_GEN_FOR_COORDS_NAMES,
								ObfHelper.createMethodDescriptor(
										obfuscatedClass,
										"net/minecraft/world/biome/Biome",
										"net/minecraft/util/math/BlockPos"));
				AbstractInsnNode insertionPoint = methodNode.instructions.get(
						methodNode.instructions.indexOf(invokeMethodNode) - 2);
		...
		if (successfulTransformations.size() != 3)
			throw new RuntimeException(
					"An error occurred transforming World. Applied transformations: "
