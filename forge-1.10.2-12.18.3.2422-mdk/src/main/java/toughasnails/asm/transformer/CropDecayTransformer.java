...
public class CropDecayTransformer implements IClassTransformer {
	private static final String[] VALID_HASHES = new String[] {
			// BlockCrops
			"3d74307bb515539176e7a84967b10a28", "b835f0bbb24031fee6ad804d8c48d2dc",
			// BlockStem
	...
	private byte[] transformToDecay(byte[] bytes, boolean obfuscatedClass, String name, boolean isVanilla) {
		System.out.println("Executing transform-to-decay");

		// Decode the class from bytes
		ClassNode classNode = new ClassNode();
		ClassReader classReader = new ClassReader(bytes);
		System.out.println("Class reader debug output:");
		...
		System.out.println("Iterating transform methodNodes");
		for (MethodNode methodNode : classNode.methods) {
			if (ASMHelper.methodEquals(methodNode, UPDATE_TICK_NAMES,
					ObfHelper.createMethodDescriptor(obfuscatedClass, "V", "net/minecraft/world/World",
				...
				insnList.add(
						new MethodInsnNode(Opcodes.INVOKESTATIC, "toughasnails/season/SeasonASMHelper", "onUpdateTick",
								ObfHelper.createMethodDescriptor(obfuscatedClass, "V", "net/minecraft/block/Block",
				...
				methodNode.instructions.insertBefore(methodNode.instructions
						.get(methodNode.instructions.indexOf(methodNode.instructions.getLast()) - 1), insnList);
		...
		if (isVanilla) {
			System.out.println("Is vanilla!");

			// The vanilla method does not exist? What is this sorcery?!?
			if (successfulTransformations.size() != 1) {
				throw new RuntimeException("An error occurred transforming " + name + ". Applied transformations: "
