...
public class ClientProxy extends CommonProxy {
	public static ResourceLocation particleTexturesLocation = new ResourceLocation(
			"toughasnails:textures/particles/particles.png");
	...
	public void registerItemVariantModel(Item item, String name, int metadata) {
		if (item != null) {
			...
			ModelLoader.setCustomModelResourceLocation(item, metadata,
					new ModelResourceLocation(ToughAsNails.MOD_ID + ":" + name, "inventory"));
		}
	}
	...
	public void registerNonRenderingProperties(Block block) {
		if (block instanceof ITANBlock) {
			...
			if (nonRenderingProperties != null) {
				// use a custom state mapper which will ignore the properties
				// specified in the block as being non-rendering
	...
	public void registerFluidBlockRendering(Block block, String name) {
		final ModelResourceLocation fluidLocation = new ModelResourceLocation(
				ToughAsNails.MOD_ID.toLowerCase() + ":fluids", name);
	...
	public void spawnParticle(TANParticleTypes type, double x, double y, double z, Object... info) {
		...
		switch (type) {
		case SNOWFLAKE:
			entityFx = new EntitySnowflakeFX(minecraft.theWorld, x, y, z,
					MathHelper.getRandomDoubleInRange(minecraft.theWorld.rand, -0.03, 0.03), -0.02D,
					MathHelper.getRandomDoubleInRange(minecraft.theWorld.rand, -0.03, 0.03));
			break;
		default:
			break;
		}
	...
	//
	// The below method and class is used as part of Forge 1668+'s workaround
	// for render manager being null during preinit
	//
