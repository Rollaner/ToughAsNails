...
public class SeasonHandler {
	...
	public static SeasonSavedData getSeasonSavedData(World world) {
		...
		SeasonSavedData savedData = (SeasonSavedData) mapStorage.getOrLoadData(SeasonSavedData.class,
	...
	public void onCropGrowPre(BlockEvent.CropGrowEvent.Pre event) {
		if (!event.getWorld().isRemote) {
			...
			if (temperatureWithering) {
				...
				if (GameplayConfigurationHandler.EXTERNAL_HIBERNATING_CROPS.containsKey(blockName)) {
				...
				} else if (block instanceof IHibernatingCrop && ((IHibernatingCrop) block).shouldHibernate()) {
					minHibernate = 5;
					minOptimal = 10;
					maxOptimal = 15;
					maxHibernate = 20;
				...
				if ((targetTemperature > maxOptimal && targetTemperature <= maxHibernate)
						|| (targetTemperature < minOptimal && targetTemperature >= minHibernate)
								&& (Math.random() < nonOptimalChance)) {
					event.setResult(Result.DENY);
				} else if (targetTemperature < minHibernate || targetTemperature > maxHibernate) {
					event.setResult(Result.DENY);
				}
			} else {
	...
	public void onCropGrowEvent(BlockEvent.CropGrowEvent event) {
		if (!event.getWorld().isRemote) {
			...
			if (temperatureWithering) {
				...
				// Alive but not optimal, slow growth
				int targetTemperature = TemperatureHandler.getTargetTemperatureAt(world, pos);
				if ((targetTemperature > maxOptimal && targetTemperature <= maxLiving)
						|| (targetTemperature < minOptimal && targetTemperature >= minLiving)
								&& (Math.random() < nonOptimalChance)) {
					event.setResult(Result.DENY);
				}
			}
