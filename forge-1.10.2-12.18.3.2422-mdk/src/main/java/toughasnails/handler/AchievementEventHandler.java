...
public class AchievementEventHandler {
	...
	public void onItemUsed(PlayerInteractEvent event) {
		/*
		 * TODO: 1.9 if (event.action != Action.LEFT_CLICK_BLOCK) { ItemStack
		 * stack = event.entityPlayer.getHeldItem(); Item item = stack != null ?
		 * stack.getItem() : null; EntityPlayer player = event.entityPlayer;
		 * 
		 * //Gone Home if (item == BOPItems.enderporter) {
	...
	/*
	 * @SubscribeEvent public void onPlayerUpdate(LivingUpdateEvent event) { if
	 * (!event.getEntity().worldObj.isRemote && event.getEntity() instanceof
	 * EntityPlayer) { EntityPlayerMP player =
	 * (EntityPlayerMP)event.getEntity();
	 * 
	 * if (player.ticksExisted % 20 * 5 == 0) { if
	 * (!player.getStatFile().hasAchievementUnlocked(TANAchievements.year_one))
	 * {
	 * 
	 * } } } }
	 */
