...
public class FillBottleHandler {
	 ...
	 */
	@SubscribeEvent
	public void onPlayerRightClickWater(
			PlayerInteractEvent.RightClickItem event) throws Exception {
		...
		if (stack != null && stack.getItem().equals(Items.GLASS_BOTTLE)
			...
			ActionResult<ItemStack> actionResult = stack.getItem()
					.onItemRightClick(new ItemStack(stack.getItem(), 0),
			...
			if (actionResult.getType().equals(EnumActionResult.SUCCESS)
					&& resultStack != null && ItemStack.areItemsEqual(
							resultStack, new ItemStack(Items.POTIONITEM))) {
	 ...
	 */
	@SubscribeEvent
	public void onRightClickCauldron(
		...
		Item heldItem;
		if (heldStack == null || state == null
				|| (heldItem = heldStack.getItem()) == null) {
			return;
		}

		if (state.getBlock() instanceof BlockCauldron
				&& heldItem == Items.GLASS_BOTTLE
				&& SyncedConfig.getBooleanValue(GameplayOption.ENABLE_THIRST)) {
			...
			if (level > 0 && !world.isRemote) {
				if (!player.capabilities.isCreativeMode) {
					ItemStack waterBottle = new ItemStack(
							TANItems.water_bottle);
...

					{
						player.setHeldItem(event.getHand(), waterBottle);
							...
							.addItemStackToInventory(waterBottle)) {
