...
public class MaxHealthHandler {
	...
	public void onServerTick(TickEvent.ServerTickEvent event) {
		...
		if (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_LOWERED_STARTING_HEALTH) && event.phase == Phase.END
	...
	public static void overrideMaximumHealth(UUID player, int newMaximumHealth) {
		if (newMaximumHealth > 0 && newMaximumHealth <= 20) {
			...
			try {
				...
				f.delete();
				f.createNewFile();
				PrintWriter writer = new PrintWriter(new FileWriter("player_health_overrides.txt"));
	...
	private void updateStartingHealthModifier(EnumDifficulty difficulty, EntityPlayer player) {
		...
		if (modifier == null || modifier.getAmount() != difficultyHealthDecrement) {
			Multimap<String, AttributeModifier> multimap = HashMultimap.<String, AttributeModifier> create();
			modifier = new AttributeModifier(HealthHelper.STARTING_HEALTH_MODIFIER_ID, "Starting Health Modifier",
					difficultyHealthDecrement, 0);
			multimap.put(SharedMonsterAttributes.MAX_HEALTH.getAttributeUnlocalizedName(), modifier);
