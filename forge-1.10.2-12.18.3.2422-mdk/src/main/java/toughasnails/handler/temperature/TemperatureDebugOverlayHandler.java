...
public class TemperatureDebugOverlayHandler {
	...
	private void drawModifierTable(int width, int height, Temperature temperature, TemperatureDebugger debugger) {
		...
		if (targetModifiers != null && rateModifiers != null) {
			...
			String targetProgress = "" + TextFormatting.RED + temperature.getRawValue() + "/"
					+ debugger.targetTemperature + getCappedText(debugger.targetTemperature);
	...
	private static void drawTable(String title, int x, int y, Map<Modifier, Integer> contents) {
		...
		Iterator<Entry<Modifier, Integer>> contentsIterator = contents.entrySet().iterator();
		...
		for (int i = 0; i < contents.size() + 1; i++) {
			...
			if (i == 0) {
				...
				fontRenderer.drawString(title, textStart + lineWidth / 2 - fontRenderer.getStringWidth(title) / 2,
						y + 1, 553648127);
			} else {
				Entry<Modifier, Integer> entry = contentsIterator.next();
				...
				fontRenderer.drawString(formattedValue, textEnd - fontRenderer.getStringWidth(formattedValue), rowTopY,
						553648127);
			}
		}
	}
	...
	@SideOnly(Side.CLIENT)
	private static int getTableHeight(Map<Modifier, Integer> contents) {
		FontRenderer fontRenderer = Minecraft.getMinecraft().fontRendererObj;
	...
	private static String getCappedText(int targetTemperature) {
		return TextFormatting.BLUE + " "
				+ (targetTemperature < 0 ? "(0)"
						: targetTemperature > TemperatureScale.getScaleTotal()
