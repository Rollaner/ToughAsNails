...
public class TemperatureOverlayHandler {
	...
	public static final ResourceLocation ICE_VIGNETTE = new ResourceLocation(
			"toughasnails:textures/gui/ice_vignette.png");
	public static final ResourceLocation FIRE_VIGNETTE = new ResourceLocation(
			"toughasnails:textures/gui/fire_vignette.png");
	...
	public void onPostRenderOverlay(RenderGameOverlayEvent.Post event) {
		...
		// When the update counter isn't incrementing, ensure the same numbers
		// are produced (freezes moving gui elements)
	...
	private void drawTemperature(int width, int height, Temperature temperature) {
		...
		if (temperatureRange == TemperatureRange.ICY || temperatureRange == TemperatureRange.HOT) {
			float shakeDelta = temperatureRange == TemperatureRange.ICY ? temperature.getRangeDelta(true)
		...
		if (flashCounter > (long) updateCounter) {
			if (updateDelta > 6 && updateDelta / 3L % 2L == 1L) {
				drawTexturedModalRect(left, top, 16 * (temperatureIcon.backgroundIndex + flashType.backgroundShift), 0,
						16, 16);
				drawTexturedModalRect(left, top, 16 * (temperatureIcon.foregroundIndex + flashType.foregroundShift), 0,
						16, 16);
	...
	private void renderColouredBall(int x, int y, Temperature temperature, int textureShift) {
		TemperatureRange temperatureRange = temperature.getRange();
		float changeDelta = temperatureRange == TemperatureRange.COOL ? temperature.getRangeDelta(true)
				: temperature.getRangeDelta(false);

		if (temperatureRange != TemperatureRange.MILD) {
			GlStateManager.pushMatrix();
			GlStateManager.color(1.0F, 1.0F, 1.0F, changeDelta);
			drawTexturedModalRect(x, y, 16 * ((temperatureRange == TemperatureRange.COOL ? 8 : 9) + textureShift),
					16 * 1, 16, 16);
	...
	private static enum TemperatureIcon {
		SNOWFLAKE(0, 9, TemperatureRange.ICY), BALL(1, 10, TemperatureRange.COOL, TemperatureRange.WARM), FIRE(2, 11,
		...
		private TemperatureIcon(int backgroundIndex, int foregroundIndex, TemperatureRange startRange,
				TemperatureRange endRange) {
