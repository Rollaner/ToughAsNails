...
public class VariantPagingHelper<B extends Block, V extends Enum<V> & IStringSerializable & VariantPagingHelper.IPagedVariants> {
	...
	public VariantPagingHelper(int variantsPerPage, Class<V> variantsEnumClass, Predicate<V> filter) {
		...
		if (variants == null) {
			throw new IllegalArgumentException(
					"Failed creating PagedBlock - variantsEnumClass has no enum constants - is it an enum class?");
		}
	...
	protected void addVariantPage(int pageNum, ArrayList<V> variants) {
		...
		this.pageNumToProperty.put(Integer.valueOf(pageNum),
				PropertyEnum.create("variant", this.variantsEnumClass, variants));
	}
	...
	public IBlockState getVariantState(V variant) {
		return this.getBlock(variant).getDefaultState().withProperty(this.getVariantProperty(this.getPageNum(variant)),
				variant);
	}
	...
	public ItemStack getVariantItem(V variant, int howMany) {
		return new ItemStack(this.getBlock(variant), howMany,
				this.getBlock(variant).getMetaFromState(this.getVariantState(variant)));
	}
