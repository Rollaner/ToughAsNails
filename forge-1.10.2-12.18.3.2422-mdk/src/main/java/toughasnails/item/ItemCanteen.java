...
public class ItemCanteen extends Item {
	public ItemCanteen() {
		this.addPropertyOverride(new ResourceLocation("filled"),
	...
	public ItemStack onItemUseFinish(ItemStack stack, World world,
		...
		if (entity instanceof EntityPlayer && waterType != null) {
			...
			ThirstHandler thirstStats = (ThirstHandler) player
					.getCapability(TANCapabilities.THIRST, null);
			thirstStats.addStats(waterType.getThirst(),
					waterType.getHydration());

			if (!world.isRemote
					&& world.rand.nextFloat() < waterType.getPoisonChance()
					&& SyncedConfig
							.getBooleanValue(GameplayOption.ENABLE_THIRST)) {
				player.addPotionEffect(
						new PotionEffect(TANPotions.thirst, 600));
			}
		}
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack stack,
		...
		if (!attemptCanteenFill(player, stack) && waterType != null
				&& getTimesUsed(stack) < 3 && thirstStats.isThirsty()) {
	 ...
	 * 
	 * @param player
	 *            The player holding the canteen.
	 * @param stack
	 *            The canteen item stack.
	...
	private boolean attemptCanteenFill(EntityPlayer player, ItemStack stack) {
		...
		if (movingObjectPos != null
				&& movingObjectPos.typeOfHit == RayTraceResult.Type.BLOCK) {
			BlockPos pos = movingObjectPos.getBlockPos();
			IBlockState state = world.getBlockState(pos);
			Fluid fluid = FluidRegistry.lookupFluidForBlock(state.getBlock());

			if (fluid != null && fluid == FluidRegistry.WATER) // Temporary,
																// until a
																// registry is
																// created
			{
