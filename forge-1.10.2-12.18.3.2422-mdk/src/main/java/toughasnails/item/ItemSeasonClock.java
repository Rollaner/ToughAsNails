...
public class ItemSeasonClock extends Item {
	public ItemSeasonClock() {
		this.addPropertyOverride(new ResourceLocation("time"),
				new IItemPropertyGetter() {
					...
					public float apply(ItemStack stack, World world,
						...
						Entity holder = (Entity) (entity != null ? entity
								: stack.getItemFrame());

						if (world == null && holder != null) {
							world = holder.getEntityWorld();
						}
						...
						if (world == null) {
							...
							if (world.provider.isSurfaceWorld()) {
								int seasonCycleTicks = SeasonHelper
										.getSeasonData(world)
										.getSeasonCycleTicks();
								d0 = (double) ((float) seasonCycleTicks
										/ (float) SeasonTime.TOTAL_CYCLE_TICKS);
							} else {
								d0 = Math.random();
							}
