...
public class ItemBottleOfGas extends Item {
	...
	// here because it determines the jar contents
	@Override
	...
	public BottleContents getContentsType(ItemStack stack) {
		...
		try {
			...
			// if metadata is out of bounds return blackdamp as a default
			// (should never happen)
			return BottleContents.BLACKDAMP;
		}
	}

	// get the correct name for this item by looking up the meta value in the
	// JarContents enum
	@Override
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos,
		...
		if (!playerIn.canPlayerEdit(pos, side, stack)) {
			...
			if (worldIn.canBlockBePlaced(this.block, pos, false, side, (Entity) null, stack)) {
				IBlockState iblockstate1 = this.block.getStateForPlacement(worldIn, pos, side, hitX, hitY, hitZ,
						this.getMetadata(stack), playerIn, stack);
