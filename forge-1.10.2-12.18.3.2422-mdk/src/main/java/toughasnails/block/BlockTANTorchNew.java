...
public class BlockTANTorchNew extends Block implements ITANBlock {
	...
	public BlockTANTorchNew() {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.UP)
				.withProperty(AGE, Integer.valueOf(0)).withProperty(BURNING, Boolean.valueOf(true)));
				...
				if (state.getValue(BURNING) == false) {
					if (!worldIn.isRainingAt(pos)) {
						if (item == Items.FLINT_AND_STEEL) {
							...
							worldIn.setBlockState(pos, TANBlocks.torch_new.getDefaultState()
									.withProperty(FACING, state.getValue(FACING)).withProperty(BURNING, true));
