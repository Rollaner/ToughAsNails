...
public class TemperatureHandler extends StatHandlerBase implements ITemperature {
	...
	public void update(EntityPlayer player, World world, Phase phase) {
		if (phase == Phase.END && !world.isRemote) {
			...
			if (debugger.targetTemperature == this.temperatureLevel)
				...
				trend = debugger.targetTemperature > this.temperatureLevel ? TemperatureTrend.INCREASING
			...
			java.util.Iterator<TemperatureModifier.ExternalModifier> iterator = this.externalModifiers.values()
					.iterator();
			...
			if ((incrementTemperature || updateClient)
				...
				Temperature targetTemperature = biomeModifier.modifyTarget(world, player,
						new Temperature(TEMPERATURE_SCALE_MIDPOINT));
				targetTemperature = altitudeModifier.modifyTarget(world, player, targetTemperature);
				...
				targetTemperature = new Temperature(
						MathHelper.clamp_int(targetTemperature.getRawValue(), 0, TemperatureScale.getScaleTotal()));

				if (incrementTemperature) {
					this.addTemperature(new Temperature(
							(int) Math.signum(targetTemperature.getRawValue() - this.temperatureLevel)));
	...
	private void addPotionEffects(EntityPlayer player) {
		...
		if (!player.capabilities.isCreativeMode && (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_TEMPERATURE))) {
			if (this.temperatureLevel <= hypoRangeStart && (!player.isPotionActive(TANPotions.cold_resistance))
					&& (temperatureLevel < prevTemperatureLevel || !player.isPotionActive(TANPotions.hypothermia))) {
				multiplier = 1.0F - ((float) (this.temperatureLevel + 1) / (float) hypoRangeSize);
				player.removePotionEffect(TANPotions.hypothermia);
				player.addPotionEffect(new PotionEffect(TANPotions.hypothermia, (int) (1800 * multiplier) + 600,
						(int) (3 * multiplier + extremityDelta)));
			} else if (this.temperatureLevel >= hyperRangeStart && (!player.isPotionActive(TANPotions.heat_resistance))
					&& (temperatureLevel > prevTemperatureLevel || !player.isPotionActive(TANPotions.hyperthermia))) {
				multiplier = (float) (this.temperatureLevel - hyperRangeStart) / hyperRangeSize;
				player.removePotionEffect(TANPotions.hyperthermia);
				player.addPotionEffect(new PotionEffect(TANPotions.hyperthermia, (int) (1800 * multiplier) + 600,
						(int) (3 * multiplier)));
			}
		}
	}
	...
	public IMessage createUpdateMessage() {
		NBTTagCompound data = (NBTTagCompound) TANCapabilities.TEMPERATURE.getStorage()
				.writeNBT(TANCapabilities.TEMPERATURE, this, null);
	...
	public void addTemperature(Temperature difference) {
		this.temperatureLevel = Math
				.max(Math.min(TemperatureScale.getScaleTotal(), this.temperatureLevel + difference.getRawValue()), 0);
	}
	...
	public void applyModifier(String name, int amount, int rate, int duration) {
		if (this.externalModifiers.containsKey(name)) {
			...
			TemperatureModifier.ExternalModifier modifier = new TemperatureModifier.ExternalModifier(name, amount, rate,
					this.temperatureTimer + duration);
	...
	public static int getTargetTemperatureAt(World world, BlockPos position) {
		...
		ObjectProximityModifier objectProximityModifier = new ObjectProximityModifier(debugger);
		...
		Temperature baseTemperature = new Temperature(TemperatureHandler.TEMPERATURE_SCALE_MIDPOINT);
		Temperature targetTemperature = biomeModifier.modifyTarget(world, position, baseTemperature);
		targetTemperature = altitudeModifier.modifyTarget(world, position, targetTemperature);
		targetTemperature = objectProximityModifier.modifyTarget(world, position, targetTemperature);
		targetTemperature = weatherModifier.modifyTarget(world, position, targetTemperature);
		targetTemperature = timeModifier.modifyTarget(world, position, targetTemperature);
		targetTemperature = seasonModifier.modifyTarget(world, position, targetTemperature);
