...
public class AltitudeModifier extends TemperatureModifier {
	...
	public AltitudeModifier(TemperatureDebugger debugger) {
		super(debugger);
		this.ALTITUDE_TARGET_MODIFIER = SyncedConfig.getIntegerValue(GameplayOption.ALTITUDE_TEMP_MODIFIER);
	}
	...
	public Temperature modifyTarget(World world, EntityPlayer player, Temperature temperature) {
		...
		if (world.provider.isSurfaceWorld()) {
			newTemperatureLevel -= MathHelper
					.abs(MathHelper.floor_double(((64 - player.posY) / 64) * ALTITUDE_TARGET_MODIFIER) + 1);
		}
	...
	public Temperature modifyTarget(World world, BlockPos position, Temperature temperature) {
		int temperatureLevel = temperature.getRawValue();
		int newTemperatureLevel = temperatureLevel;

		if (world.provider.isSurfaceWorld()) {
			newTemperatureLevel -= MathHelper
					.abs(MathHelper.floor_double(((64 - position.getY()) / 64) * ALTITUDE_TARGET_MODIFIER) + 1);
		}
