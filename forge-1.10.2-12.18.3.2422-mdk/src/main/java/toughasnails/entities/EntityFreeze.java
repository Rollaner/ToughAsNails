...
public class EntityFreeze extends EntityMob implements IMob {
	...
	/*
	 * protected String getLivingSound() { return "mob.freeze.breathe"; }
	 * 
	 * protected String getHurtSound() { return "mob.freeze.hit"; }
	 * 
	 * protected String getDeathSound() { return "mob.freeze.death"; }
	 */
	...
	public void onLivingUpdate() {
		...
		if (this.getEntityWorld().isRemote) {
			 ...
			 * this.posZ + 0.5D, "fire.fire", 1.0F + this.rand.nextFloat(),
			 * this.rand.nextFloat() * 0.7F + 0.3F, false); }
			 */

			for (int i = 0; i < 2; ++i) {
				ToughAsNails.proxy.spawnParticle(TANParticleTypes.SNOWFLAKE,
	...
	static class AIFireballAttack extends EntityAIBase {
		...
		public void updateTask() {
			...
			if (d0 < 4.0D) {
				...
				this.freeze.getMoveHelper().setMoveTo(entitylivingbase.posX, entitylivingbase.posY,
				...
				if (this.field_179468_c <= 0) {
					...
					if (this.field_179467_b > 1) {
						float f = MathHelper.sqrt_double(MathHelper.sqrt_double(d0)) * 0.5F;
						this.freeze.getEntityWorld().playEvent((EntityPlayer) null, 1009,
								new BlockPos((int) this.freeze.posX, (int) this.freeze.posY, (int) this.freeze.posZ),
								0);

						for (int i = 0; i < 1; ++i) {
							EntityIceball entityiceball = new EntityIceball(this.freeze.getEntityWorld(), this.freeze,
									d1 + this.freeze.getRNG().nextGaussian() * (double) f, d2,
									d3 + this.freeze.getRNG().nextGaussian() * (double) f);
							entityiceball.posY = this.freeze.posY + (double) (this.freeze.height / 2.0F) + 0.5D;
							this.freeze.getEntityWorld().spawnEntityInWorld(entityiceball);
						}
					}
				}
