...
public class EntityIceball extends Entity implements IProjectile {
	...
	public void onUpdate() {
		if (this.getEntityWorld().isRemote || (this.shootingEntity == null || !this.shootingEntity.isDead)
				&& this.getEntityWorld().isBlockLoaded(new BlockPos(this))) {
			super.onUpdate();

			if (this.inGround) {
				if (this.getEntityWorld().getBlockState(new BlockPos(this.xTile, this.yTile, this.zTile))
			...
			if (movingobjectposition != null) {
				vec31 = new Vec3d(movingobjectposition.hitVec.xCoord, movingobjectposition.hitVec.yCoord,
						movingobjectposition.hitVec.zCoord);
			}

			Entity entity = null;
			List<Entity> list = this.getEntityWorld().getEntitiesWithinAABBExcludingEntity(this,
					this.getEntityBoundingBox().addCoord(this.motionX, this.motionY, this.motionZ).expand(1.0D, 1.0D,
							1.0D));
			double d0 = 0.0D;
			...
			for (this.rotationPitch = (float) (MathHelper.atan2((double) f1, this.motionY) * 180.0D / Math.PI)
			...
			if (this.isInWater()) {
				for (int j = 0; j < 4; ++j) {
					...
					this.getEntityWorld().spawnParticle(EnumParticleTypes.WATER_BUBBLE,
							this.posX - this.motionX * (double) f3, this.posY - this.motionY * (double) f3,
							this.posZ - this.motionZ * (double) f3, this.motionX, this.motionY, this.motionZ,
							new int[0]);
				}
			...
			ToughAsNails.proxy.spawnParticle(TANParticleTypes.SNOWFLAKE, this.posX, this.posY + 0.5D, this.posZ, 0.0D,
					0.0D, 0.0D, new int[0]);
			this.setPosition(this.posX, this.posY, this.posZ);
		} else {
			this.setDead();
		...
		if (!this.getEntityWorld().isRemote) {
			if (movingObject.entityHit != null) {
				...
				if (this.shootingEntity != null && this.shootingEntity instanceof EntityLiving) {
					flag1 = this.getEntityWorld().getGameRules().getBoolean("mobGriefing");
				}

				if (flag1) {
					// BlockPos blockpos = movingObject.getBlockPos()
					// .offset(movingObject.sideHit);

					/*
					 * if (this.worldObj.isAirBlock(blockpos)) {
					 * this.worldObj.setBlockState(blockpos,
