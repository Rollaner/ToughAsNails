...
public class TANCommand extends CommandBase {
	...
	private void displayTemperatureInfo(ICommandSender sender, String[] args) throws CommandException {
		...
		TemperatureHandler temperatureStats = (TemperatureHandler) player.getCapability(TANCapabilities.TEMPERATURE,
				null);
	...
	private void retrieveTemperatureAt(ICommandSender sender, String[] args) throws CommandException {
		int dimensionID = 0;
		int x = 0;
		int y = 0;
		int z = 0;
		...
		if (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_TEMPERATURE)) {
			...
			WorldServer candidate = FMLCommonHandler.instance().getMinecraftServerInstance()
			...
			int finalTemperature = TemperatureHandler.getTargetTemperatureAt(world, position);
			if (printOutput) {
				sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.tempat.success", dimensionID,
						x, y, z, finalTemperature));
			}
			sender.setCommandStat(CommandResultStats.Type.QUERY_RESULT, finalTemperature);
		} else {
			sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.tempat.disabled"));
		}
	}
	...
	private void setSeason(ICommandSender sender, String[] args) throws CommandException {
		...
		if (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_SEASONS)) {
			if (newSeason != null) {
				...
				seasonData.seasonCycleTicks = SeasonTime.DAY_TICKS * SeasonTime.SUB_SEASON_DURATION
						* newSeason.ordinal();
				seasonData.markDirty();
				SeasonHandler.sendSeasonUpdate(world);
				if (printOutput) {
					sender.addChatMessage(
							new TextComponentTranslation("commands.toughasnails.setseason.success", args[1]));
				}
			} else {
	...
	private void setHealth(ICommandSender sender, String[] args) throws CommandException {
		...
		if (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_LOWERED_STARTING_HEALTH)) {
			...
			// Set the new health
			MaxHealthHandler.overrideMaximumHealth(player.getUniqueID(), newHealth);

			sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.sethealth.success", newHealth));
		} else {
			sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.sethealth.disabled"));
		}
	}
	...
	private void setThirst(ICommandSender sender, String[] args) throws CommandException {
		...
		if (SyncedConfig.getBooleanValue(GameplayOption.ENABLE_THIRST)) {
			...
			sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.setthirst.success", thirstLevel));
		} else {
			sender.addChatMessage(new TextComponentTranslation("commands.toughasnails.setthirst.disabled"));
		}
	}
